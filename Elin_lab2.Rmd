...
author: " Elin Palm, Saga Bolund" # REPLACE WITH YOUR OWN NAME(S) 
...
# Write your code / answer here
unique_rna <- as.numeric(se$nFeature_RNA)

hist(unique_rna, breaks = 70)

abline(v = mean(unique_rna), col = "black", lty = "longdash", lwd = 2)
# 1st standard deviation
abline(v = mean(unique_rna) - sd(unique_rna), col = "darkred", lty = "longdash", lwd = 2)
abline(v = mean(unique_rna) + sd(unique_rna), col = "darkred", lty = "longdash", lwd = 2)
# 2nd standard deviation
abline(v = mean(unique_rna) - 2*sd(unique_rna), col = "red", lty = "longdash", lwd = 2)
abline(v = mean(unique_rna) + 2*sd(unique_rna), col = "red", lty = "longdash", lwd = 2)
...
VlnPlot(se, features = c("nFeature_RNA",'nCount_RNA'), group.by = "replicate") +
  geom_hline(aes(yintercept = 5000, color = "threshold"), linetype = "longdash") +
  labs(color = "") + # Remove title from color legend
...
set3 <- gene.counts > 500


# Select cells where both criteria above are met (set1 AND set2 has to be TRUE)
keep.genes <- rownames(se)[set3]
head(keep.genes)
...
# Write your code / answer here
Number_of_cells<-ncol(se.subset)
Number_of_gene<-nrow(se.subset)
...
# Write your code / answer here
bc_0.3 <- FindClusters(bc, resolution = 0.3)
bc_1.2<- FindClusters(bc, resolution = 1.2)
...
FeaturePlot(bc, dims=c(1,2), features = c("MS4A1", "KIT", "CD14"))
```
...
#top2.markers <- top_n(n =2, wt = avg_log2FC)
...
top1.markers <- bc.markers %>% 
  group_by(cluster) %>% # This will group the results of bc.markers by cluster
  top_n(n = 1, wt = avg_log2FC) # This will select the top 2 genes with respect to

FeaturePlot(bc, features = top1.markers$gene)
```
...
Tcell_markers <- subset(bc.markers, cluster == "2") %>%
  filter(p_val_adj < 0.01, avg_log2FC > 0.5) %>%
  pull(gene)

# Run FEA
Tcell_res <- gost(query = Tcell_markers, 
            organism = "hsapiens", 
            significant = TRUE, 
            sources = "GO:BP")

# Check results
head(Tcell_res$result, 10)
...
term_name <- 'T cell activation'
p_value <- 2.614639e-09
...
p1 <- FeaturePlot(bc, features = "CD3E")
p2 <- FeaturePlot(bc, features = c("S.Score","G2M.Score"))
p3 <- VlnPlot(bc, features = 'MKI67', group.by = "CellType")
...
q13.answer <- c('PVL cells', 'pDCs', 'plasmablasts', 'Endothelial 2')
